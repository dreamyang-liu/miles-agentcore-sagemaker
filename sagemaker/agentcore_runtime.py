...
RUNTIME_NAME = "miles_math_agent_vpc"
INFRA = json.loads(Path(__file__).with_name(".infra.json").read_text())
STATE = Path(__file__).with_name(".agentcore_runtime.json")
...
def _delete_runtime(runtime_id: str) -> None:
    ...
    while _find_runtime(RUNTIME_NAME):
        time.sleep(5)
...
def create(args: argparse.Namespace) -> None:
    ...
    existing = _find_runtime(RUNTIME_NAME)
    if existing and existing["status"].endswith("_FAILED"):
        _delete_runtime(existing["agentRuntimeId"])  # cannot be repaired in place
        existing = None
    ...
    for attempt in range(1, 4):
        ...
        _delete_runtime(rt["agentRuntimeId"])
        existing = None
